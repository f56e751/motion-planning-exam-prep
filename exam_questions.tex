...
\subsection*{[보조2] Discrete costate $\to$ Continuous PMP 극한 \hfill ★★★★}
...
$\Delta t \to 0$ 극한을 취해 연속시간 \textbf{Pontryagin 최소원리} 식
\[
...
\begin{sol}
\textbf{Step 1 --- Jacobian 대입}. $F = I\cdot x + \Delta t\, f$이므로
\[
F_x = I + \Delta t\, f_x, \qquad F_u = \Delta t\, f_u.
\]

\textbf{Step 2 --- Costate 식 정리}. 대입하면
\[
...
\textbf{Step 3 --- Limit $\Delta t \to 0$}. $k\,\Delta t \to t$로 두고 $\lambda_k \to \lambda(t)$. 좌변의 backward difference는 $-\dot\lambda(t)$:
\[
-\dot\lambda = \ell_x + f_x^T \lambda.
\]
$H = \ell + \lambda^T f$이면 $\partial_x H = \ell_x + f_x^T \lambda$:
\[
\boxed{\dot\lambda(t) = -\frac{\partial H}{\partial x}.}
\]

\textbf{Step 4 --- Control optimality}. $\Delta t\,\ell_u + (\Delta t\, f_u)^T \lambda_{k+1} = 0$ 양변을 $\Delta t$로 나누면
\[
\ell_u + f_u^T \lambda = 0 \quad\Leftrightarrow\quad \boxed{\frac{\partial H}{\partial u} = 0.}
\]

\textbf{해석}: 이산 NMPC의 FONC가 곧 PMP의 이산화. costate $\lambda$는 상태가 미래 cost에 미치는 sensitivity로, Hamiltonian의 $x$-편미분에 음의 부호로 역방향 전파. DDP backward pass는 이 PMP의 직접 이산 구현이다.
\end{sol}
...
총 \textbf{29문제} (L05, L06, L12에 보조2 추가).
...
12 & Costate recursion 유도              & ★★★★  & PMP on LQR / \textbf{Discrete→Cont PMP 극한} & ★★ / \textbf{★★★★} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}\setcounter{enumi}{2}
  ...
  \item \textbf{L12 보조2} Discrete $\to$ Continuous PMP 극한 --- NMPC $\leftrightarrow$ PMP 본질.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{유도 문제 1--2개}: L06 min-jerk 차수 / L05 chained form / L12 costate or PMP 극한 중.
...
총 \textbf{29문제}. 풀이는 cheat sheet 없이도 따라갈 수 있도록 단계별. 손계산 가능한 수치만 사용.
